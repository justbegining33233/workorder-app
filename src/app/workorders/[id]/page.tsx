...
export default function WorkOrderDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Estimate handlers
  const handleEstimateBuilderSave = (estimateData: any) => {
    updateWorkOrder({
      estimate: {
        amount: estimateData.total,
        details: estimateData.notes,
        status: 'proposed',
  ...
  const handleBarcodeScan = (barcode: string) => {
    setNewPart({...newPart, name: barcode});
    ...
    <div style={{minHeight:'100vh', background: 'transparent'}}>
                  ...
                  {['tech', 'manager'].includes(role) && (
                    <div id="addPartForm" style={{display:'none', padding:12, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:6, marginBottom:8}}>
                      <div style={{display:'flex', gap:8, marginBottom:8}}>
                        <input
                          type="text"
                          placeholder="Part name or scan barcode"
                          value={newPart.name}
                          onChange={(e) => setNewPart({...newPart, name: e.target.value})}
                          style={{flex:1, padding:'6px 8px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.2)', borderRadius:4, color:'#e5e7eb', fontSize:12}}
                        />
                        <button
                          type="button"
                          onClick={() => setShowBarcodeScanner(true)}
                          style={{padding:'6px 8px', background:'#3b82f6', color:'white', border:'none', borderRadius:4, cursor:'pointer', fontSize:12}}
                          title="Scan barcode"
                        >
                          <FaCamera />
                        </button>
      ...
      {showEstimateBuilder && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16}}>
          <EstimateBuilder
            workOrderId={id || ''}
            onSave={handleEstimateBuilderSave}
